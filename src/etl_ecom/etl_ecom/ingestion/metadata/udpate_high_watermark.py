...
def update_high_watermark(table: str, warehouse_engine: DuckDBPyConnection) -> None:
    ...
    try:
        ...
        if not sql_query:
            raise ValueError("❌ Could not load upsert_watermark.sql")

        # 🚀 upsert watermark
        warehouse_engine.execute(sql_query, [table, new_watermark, int(last_id)])

        logger.info(
            f"✅ Watermark updated for {table}: " f"{new_watermark} (id={last_id})"
        )
